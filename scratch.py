...
if __name__ == '__main__':
  ...
  model.add(Flatten())
  model.add(Dense(OUTPUT_SIZE*6, activation='relu'))
  ...
  for i in range(0, SAMPLE_SIZE):
    x = np.array([np.array([np.random.uniform() for j in range(0, INPUT_SIZE)]) for j in range(0, SEQUENCE_LENGTH)])
    z, t = score_vectore(x[0])
    y = np.array([z, t])
    X.append(x)
    Y.append(y)
  ...
  model.fit( X, Y, batch_size=100, verbose=1, epochs=10)
